XsubMFitFermGroups<-function(x, free, IEactive, Soil, Treatment){
  #model parameters
  m_pars <- c("Im", "KmG", "KmA", "Gm", "edemand", "psi", "eta", "k") #
  
  if((0 %in% free) == T){
    all_p <- as.numeric(rep(x[1:(length(x)-1)], times = 2))
  }else{
    ...
    m_parsfixed <- x[(length(free)*2 + 1):(length(x)-1)]
  ...
  y0 <- c(500, 0, 0, 0, 0, 0, 0, 0, 1e-8, 1e-8, BUF0, 1e-8, 1e-8, BUB0, 0, 1e-8)
  ...
  ##==========================Running model
  ModelOut <- odeint(XsubMFermGroups, y0, times, args=tuple(all_p))
  SimOut <- odeint(XsubMFermGroups, y0, timesSim, args=tuple(all_p))
  ...
  Yhat <- matrix(data = c(ModelOut[, 1], #glucose concentration
                          ...
                          (ModelOut[, 4] + ModelOut[, 7])*conversions[1] + 
                            ((ModelOut[, 4]+ ModelOut[, 11])*(ModelOut[, 2] + ModelOut[, 3]) + 
                               (ModelOut[, 7] + ModelOut[, 14])*(ModelOut[, 5] + ModelOut[, 6]))*conversions[2], #CLC
                          -log10(H0+extraH+(((ModelOut[, 15] + ModelOut[, 16])/2*0.28/(1-0.28)/1e3)*1.75e-5/(10^-mpH + 1.75e-5)))), #pH - 1mol of acetic acid contains 2 mols C so ModelOut[, 11]/2
  ...
  Sim <- data.frame(Gl = SimOut[, 1], #glucose concentration
                  ...
                  Cflush = (SimOut[, 4] + SimOut[, 7])*conversions[1] + 
                    ((SimOut[, 4]+ SimOut[, 11])*(SimOut[, 2] + SimOut[, 3]) + 
                       (SimOut[, 7] + SimOut[, 14])*(SimOut[, 5] + SimOut[, 6]))*conversions[2], #CLC
                  pH = -log10(H0+extraHsim+(((SimOut[, 15] + SimOut[, 16])/2*0.28/(1-0.28)/1e3)*1.75e-5/(10^-mpHsim + 1.75e-5))),
                  Time = timesSim,

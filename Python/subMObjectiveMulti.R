subMObjectiveMulti<-function(i){#, free
  #all parameters that can be potentially free
  free_p <- c("ImF", "ImB", "KmF", "KmB", "yAF", "yAB", "GmF", "GmB", "kF", "kB", "edemandF", "edemandB", "etaF", "etaB") #
  ...
  var_p_x <- x[i, 1:(length(var_p))]
  names(var_p_x) <- var_p
  #adding variable parameters to all_p 
  all_p[!is.na(match(names(all_p), names(var_p_x)))] <- var_p_x
  
  #fixed parameters in x
  fixed_p_x <- rep(x[i, ((length(var_p))+1):(length(x[i, ])-1)], each = 2) #last is nb - same for all iterations (microbial community independent)
  ...
  #all_p <- append(all_p, tail(x[i, ], 3))
  #=============================================================================
  #=====================distinguish two sets of parameters
  #inherent model parameters - Fungal Im, Km, yA, G, m, edemand, emax + Bacterial Im, yA, G, m, edemand, emax
  model_pars <- as.numeric(all_p[1:14])
  ...
  BUF0 = Bunlab0*8.67e-3
  BUB0 = Bunlab0*(1 - 8.67e-3)
  ...
  y0 <- c(500, 0, 0, 0, 0, 0, 0, 0, BUF0, BUB0, 0, 0, 0)
